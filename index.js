...
function validateTradingData(ohlc, prevCandles, indicators, volume, avgVolume, keyLevels, timeframe) {
  ...
  if (!prevCandles || !Array.isArray(prevCandles)) {
    throw new Error("prevCandles must be an array");
  }
...
function isHighImpactNews(marketContext, symbol) {
  try {
    ...
    return marketContext.newsEvents.some(event => {
      ...
      return symbol.includes(event.currency) && 
             event.impact === "high" &&
...
function calculateRiskReward(ohlc, keyLevels) {
  try {
    ...
    const potentialReward = Math.max(0, keyLevels.r1 - ohlc.close);
    const potentialRisk = Math.max(0, ohlc.close - keyLevels.s1);
